<?php
/*
Plugin Name: WYSIWYG Editing
Plugin URI: http://wordpress.org/#
Description: This plugin activates WYSIWYG (what you see is what you get) editing for different fields in WordPress. It is based on the <a href="http://www.themaninblue.com/experiment/widgEditor/">widgEditor</a>.
Author: Matt Mullenweg
Version: 1.0
Author URI: http://photomatt.net/
*/ 

if ( isset( $_GET['file'] ) && 'wysiwyg.php' == basename($_SERVER['PHP_SELF']) ) :
switch ( $_GET['file'] ) :

case 'widgContent.css' :
...
{
	background-image: url(wysiwyg-images/widgeditor_button_shadow.gif);
	background-repeat: no-repeat;
...
.widgButtonBold
{
	background-image: url(wysiwyg-images/widgeditor_button_bold.gif);
}

.widgButtonHTML
{
	background-image: url(wysiwyg-images/widgeditor_button_html.gif);
}

.widgButtonImage 
{
	background-image: url(wysiwyg-images/widgeditor_button_image.gif);
}

.widgButtonItalic 
{
	background-image: url(wysiwyg-images/widgeditor_button_italic.gif);
}

.widgButtonLink
{
	background-image: url(wysiwyg-images/widgeditor_button_link.gif);
}

.widgButtonOrdered
{
	background-image: url(wysiwyg-images/widgeditor_button_ordered.gif);
}

.widgButtonUnordered
{
	background-image: url(wysiwyg-images/widgeditor_button_unordered.gif);
}

.widgSource a
{
	background-image: url(wysiwyg-images/widgeditor_button_empty.gif);
	cursor: default;
}
...
.widgToolbar
{
	...
	background-image: url(wysiwyg-images/widgeditor_toolbar_bg.gif);
...
var widgStylesheet = "../wp-content/plugins/wysiwyg.php?file=widgContent.css";
...
<style type="text/css" media="all">
	@import "../wp-content/plugins/wysiwyg.php?file=widgEditor.css";
</style>

<script type="text/javascript" src="../wp-content/plugins/wysiwyg.php?file=widgEditor.js"></script>
	';
	ob_start('wysi_replace');
}

function wysi_replace( $text ) {
	$text = str_replace('name="content" ', 'name="content" class="widgEditor" ', $text);
	$text = preg_replace('|<div id="quicktags">.*?</div>|s', '', $text);
...
function remove_single_breaks( $text ) {
	$text = str_replace(array("\r\n", "\r"), "\n", $text); // cross-platform newlines 
	$text = str_replace("\n", ' ', $text);
	return $text;
}

function wysi_clean( $text ) {
	$text = str_replace('&nbsp;', ' ' , $text);
	$text = preg_replace('|<p>\s*|s', "<p>", $text);
	$text = preg_replace('|\s*</p>|s', "</p>\n\n", $text);
	$text = str_replace('<p></p>', '', $text);
	$text = preg_replace('|<p>(.*?)</p>|ise', "'<p>'. remove_single_breaks('$1') . '</p>'", $text); 
	return $text;
}

add_filter('content_save_pre', 'wysi_clean');

add_action('admin_head', 'wysi_head');
